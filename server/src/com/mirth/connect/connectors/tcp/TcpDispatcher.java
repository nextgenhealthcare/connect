...
public class TcpDispatcher extends DestinationConnector {
	...
	public void replaceConnectorProperties(ConnectorProperties connectorProperties, ConnectorMessage connectorMessage) {
		...
		tcpSenderProperties
				.setRemoteAddress(replacer.replaceValues(tcpSenderProperties.getRemoteAddress(), connectorMessage));
		tcpSenderProperties
				.setRemotePort(replacer.replaceValues(tcpSenderProperties.getRemotePort(), connectorMessage));
		tcpSenderProperties
				.setLocalAddress(replacer.replaceValues(tcpSenderProperties.getLocalAddress(), connectorMessage));
	...
	public void onStop() throws ConnectorTaskException {
		...
		for (String socketKey : connectedSockets.keySet().toArray(new String[connectedSockets.size()])) {
			try {
				...
				if (firstCause == null) {
					firstCause = new ConnectorTaskException("Error closing socket (" + connectorProperties.getName()
	...
	public void onHalt() throws ConnectorTaskException {
		...
		for (String socketKey : timeoutThreads.keySet().toArray(new String[timeoutThreads.size()])) {
			try {
				...
				Thread.currentThread().interrupt();
				firstCause = new ConnectorTaskException(
						"Thread join operation interrupted (" + connectorProperties.getName() + " \""
	...
	public Response send(ConnectorProperties connectorProperties, ConnectorMessage message) {
		...
		try {
			...
			// If keep connection open is true, then interrupt the thread so it won't close
			// the socket
			if (tcpDispatcherProperties.isKeepConnectionOpen() && timeoutThread != null) {
				disposeThreadQuietly(socketKey);
			}

			// Initialize a new socket if our current one is invalid, the remote side has
			// closed, or keep connection open is false
			...
			if (!tcpDispatcherProperties.isIgnoreResponse()) {
				...
				try {
					...
					// We only want to validate the response if we were able to retrieve it
					// successfully
				...
				try {
					// MIRTH-2980: Since we're ignoring responses, flush out the socket's input
					// stream so it doesn't continually grow
			...
			if (tcpDispatcherProperties.isKeepConnectionOpen()
					&& (getCurrentState() == DeployedState.STARTED || getCurrentState() == DeployedState.STARTING)) {
				if (sendTimeout > 0) {
			...
			// If an exception occurred then close the socket, even if keep connection open
			// is true
			...
			eventController.dispatchEvent(new ConnectorCountEvent(getChannelId(), getMetaDataId(), getDestinationName(),
					ConnectionStatusEventType.IDLE,
					SocketUtil.getLocalAddress(socket) + " -> " + SocketUtil.getInetAddress(socket), (Boolean) null));
		}
	...
	private void closeSocketQuietly(String socketKey) {
		try {
			...
			logger.debug("Error closing socket (" + connectorProperties.getName() + " \"" + getDestinationName()
					+ "\" on channel " + getChannelId() + ").", e);
		}
	}
	...
	private void closeSocket(String socketKey) throws IOException {
		...
		if (socket != null) {
			...
			try {
				if (wasOpen) {
					logger.trace("Closing socket (" + connectorProperties.getName() + " \"" + getDestinationName()
							+ "\" on channel " + getChannelId() + ").");
	...
	private void disposeThreadQuietly(String socketKey) {
		try {
			...
			logger.warn("Thread join operation interrupted (" + connectorProperties.getName() + " \""
					+ getDestinationName() + "\" on channel " + getChannelId() + ").", e);
		}
	}
	...
	/*
	 * Returns the byte array representation of the connector properties template,
	 * using the properties to determine whether or not to encode in Base64, and
	 * what charset to use.
	 */
	private byte[] getTemplateBytes(TcpDispatcherProperties tcpSenderProperties, ConnectorMessage connectorMessage)
			throws UnsupportedEncodingException {
		byte[] bytes = new byte[0];

		if (tcpSenderProperties.getTemplate() != null) {
			bytes = getAttachmentHandlerProvider().reAttachMessage(tcpSenderProperties.getTemplate(), connectorMessage,
					CharsetUtils.getEncoding(tcpSenderProperties.getCharsetEncoding()),
					tcpSenderProperties.isDataTypeBinary(),
					tcpSenderProperties.getDestinationConnectorProperties().isReattachAttachments());
		}
