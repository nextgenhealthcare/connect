...
public class MessageObjectController {
	...
	public List<MessageObject> getMessagesByPage(int page, int pageSize) throws ControllerException {
		...
		try {
			...
			query.append("select messages.* from messages_work, messages where messages_work.id = messages.sequence_id");
			
			if ((page != -1) && (pageSize != -1)) {
				int first = page * pageSize;
				int last = first + pageSize;
				query.append("  and messages_work.order_seq between " + first + " and " + last + ";");
			}
	...
	public void createTempMessagesTable(MessageObjectFilter filter) throws ControllerException {
		...
		try {
			dbConnection = DatabaseConnectionFactory.createDatabaseConnection();
			dbConnection.executeUpdate("drop table messages_work;");
			dbConnection.executeUpdate("create table messages_work(id integer, order_seq integer generated by default as identity);");
			dbConnection.executeUpdate("create index index_messages_work on messages_work(order_seq, id);");
			
			StringBuilder insert = new StringBuilder();
			insert.append("insert into messages_work(id) ");
