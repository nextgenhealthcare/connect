...
public class DelimitedReader extends ParserAdapter {
	...
	public void parse(InputSource input) throws SAXException, IOException {
		...
		// Parsing overview
		//
		// The incoming stream is a single message which is a collection of one or more records.
		// Each record is a collection of columns.  Columns can be either fixed width, or delimited.
		// Records are delimited.
		//
		// Assumptions
		// 1. Fixed record layout
		//    Each record is assumed to contain the same number and type of columns.
		// 2. Special characters are single characters.  This includes: column delimiter, 
		//	  record delimiter, quote character, quote escape character.
		//
		// The following user configurable options affect the behavior of the parser:
		// o columnWidths			The array of fixed column widths.
		// o columnDelimiter		The character that delimits (separates) the columns.
		// o recordDelimiter		The character that delimits (separates) each record.
		// o quoteChar				The character that is used to quote a column value.
		// o escapeWithDoubleQuote	Iff true, embedded quotes are escaped with two consecutive quotes.
		//							  Otherwise, the quote escape character escapes embedded quote characters.
		// o quoteEscapeChar		The character used to escape a quote (or itself).
		// o ignoreCR				Iff true, all incoming \r characters are ignored and not processed.
		//
		// The following user configurable options affect the behavior of the output:
		// o columnNames			A list of column names (taken from either file header, or supplied by user). 
	...
	public String getMessage(BufferedReader in, boolean skipHeader) throws IOException {
		...
		if (props.isBatchSplitByRecord()) {
		...
		else if (DelimitedProperties.isSet(props.getBatchGroupingColumn())) {
			// Each message is a collection of records with the same value in the specified column.
			// The end of the current message occurs when a transition in the value of the specified
			// column occurs.
			
			// Prime the pump: get the first record, and save the grouping column. 
			ArrayList<String> record = getRecord(in, message);
			...
			if (record != null) {
				...
				// Read records until the value in the grouping column changes or there are no more records
				for (;;) {
	 ...
	 *  
	 * @param in The input stream (it's a BufferedReader, because operations on it require in.mark()).
	 * @param rawText Optional StringBuilder used to return a copy of the raw text read by this method.
	 * @return The record represented as a collection of column values, or null if there is no next record.
	 * @throws IOException
	 */
	private ArrayList<String> getRecord(BufferedReader in, StringBuilder rawText) throws IOException {

		// If there is an ungotten (pushed back) record, consume it and return it, rather
		...
		if (props.getColumnWidths() != null) {
			...
			// Consume trailing characters up until end of input stream or record delimiter
	...
	/**
	 * Unget the given record.  The next call to getRecord() will consume it, and return it,
	 * rather than operating on the input stream.
	 * 
	 * @param record The record to unget.
	 * @param rawText The raw text for the record to unget.
	 */
	 ...
	 * @param in The input stream (it's a BufferedReader, because operations on it require in.mark()).
	 * @param rawText Optional StringBuilder used to return a copy of the raw text read by this method.
	 * @return The next character read from the input stream, or -1 if the end of input stream is reached.
	 * @throws IOException
	 */
	 ...
	 * @return If the input string has trailing whitespace, a new string with the whitespace removed, otherwise, return s.
	 */
