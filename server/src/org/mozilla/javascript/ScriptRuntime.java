...
public class ScriptRuntime {
    ...
    private static Logger logger = Logger.getLogger(ScriptRuntime.class);
    ...
    private static boolean eqString(CharSequence x, Object y)
    {
        for (;;) {
            if (y == null || y == Undefined.instance) {
                ...
                } else if (y instanceof NativeJavaObject) {
                    // TODO: Remove in 3.1
                    // This entire file can be deleted when this is no longer needed, since we're only overridding ScriptRuntime to perform the below code.
                    Object obj = ((NativeJavaObject) y).unwrap();
                    if (obj.getClass().getName().equals("com.mirth.connect.donkey.model.message.Status")) {
                        String status = obj.toString();
                        if (x.equals("SUCCESS")) {
                            logger.error("The \"SUCCESS\" status is no longer available. Please compare statues using \"SENT\" instead.");
                            return status.equals("SENT");
                        } else if (x.equals("FAILURE")) {
                            logger.error("The \"FAILURE\" status is no longer available. Please compare statues using \"ERROR\" instead.");
                            return status.equals("ERROR");
                        } else if (x.equals("ACCEPTED") || x.equals("UNKNOWN")) {
                            logger.error("The \"" + x + "\" status is no longer available. The available statuses are: RECEIVED, FILTERED, TRANSFORMED, SENT, QUEUED, ERROR, and PENDING.");
                        }
                    }
                }
                y = toPrimitive(y);
