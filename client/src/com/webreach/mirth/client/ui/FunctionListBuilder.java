...
public class FunctionListBuilder {
	...
	public ArrayList<FunctionListItem> getVariableListItems() {
		...
		variablelistItems.add(new FunctionListItem("Send an Email", "Sends an alert email using the alert SMTP properties.", "var smtpConn = SMTPConnection.createSMTPConnection();\nsmtpConn.send('to', 'cc', 'from', 'subject', 'body');"));
		variablelistItems.add(new FunctionListItem("Perform Database Query", "Performs a database query and returns the resultset.", "var conn = DatabaseConnectionFactory.createDatabaseConnection('driver', 'address', 'username', 'password');\nvar result = conn.executeQuery('expression');\nconn.close();"));
