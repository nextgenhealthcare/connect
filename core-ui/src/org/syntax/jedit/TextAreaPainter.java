...
public class TextAreaPainter extends JComponent implements TabExpander {
	...
	public TextAreaPainter(JEditTextArea textArea, TextAreaDefaults defaults, Font font) {
		...
		eolMarkers = textArea.isShowLineEndings();
	}
	...
	protected void paintPlainLine(Graphics gfx, int line, Font defaultFont, Color defaultColor, int x, int y) {
		...
		if (eolMarkers && line < textArea.getLineCount() - 1) {
			gfx.setColor(eolMarkerColor);
			char[] chars = textArea.getLineTextWithEOL(line).toCharArray();

			if (chars.length > 1) {
				char eol = chars[chars.length - 2];
				if (eol == '\r') {
					gfx.drawString("\\r", x, y);
					x = x + fm.charWidth('w') * 2;
				} else if (eol == '\n') {
					gfx.drawString("\\n", x, y);
					x = x + fm.charWidth('w') * 2;
				}
