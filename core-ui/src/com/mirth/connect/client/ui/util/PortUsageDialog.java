...
public class PortUsageDialog extends MirthDialog {
    ...
    private void setPorts() {
    	...
    	try {
			ports = parent.getClient().getChannelPortsInUse();
        ...
        for (int i = 0; i < ports.size(); i++) {
			...
			try {
	            ...
	            if (enabled == "Enabled") {
	            	enabled = parent.getClient().getChannelStatus(port.getId().toString()).getState().toString();
	            }
			} catch (ClientException e) {
				// TODO Auto-generated catch block
				//e.printStackTrace();
    ...
    public Channel getChannelById(String id) throws ClientException {
        for (Channel channel : parent.getClient().getAllChannels()) {
