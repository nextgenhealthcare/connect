...
public class StringUtil {
    ...
    /**
     * A memory efficient method for getting bytes from a string. It first calculates the required byte array size to avoid any buffers growing out of control.
     * The main purpose of the method is to alleviate the use case when charset = "UTF-8". For some reason, using new String(string, "UTF-8") requires
     * memory many times the size of the string itself. There is a performance decrease over the Java method of about 30-100%, so this method would ideally only
