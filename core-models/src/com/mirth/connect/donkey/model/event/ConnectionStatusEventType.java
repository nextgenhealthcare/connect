...
public enum ConnectionStatusEventType {
    IDLE(true), READING(true), WRITING(true), POLLING(true), RECEIVING(true), SENDING(
            true), WAITING_FOR_RESPONSE(true), CONNECTED(
                    true), CONNECTING(true), DISCONNECTED(false), INFO(false), FAILURE(false);

    private static ConnectionStatusEventType[] priority = new ConnectionStatusEventType[] { SENDING,
            RECEIVING, WRITING, READING, POLLING, CONNECTED, WAITING_FOR_RESPONSE, CONNECTING,
            IDLE };
    ...
    static public ConnectionStatusEventType compare(ConnectionStatusEventType status1, ConnectionStatusEventType status2) {
        if (status1 == null && status2 != null)
            return status2;
        else if (status2 == null && status1 != null)
            return status1;
