...
<jsp:include page="/_jsp/header.jsp">
	<jsp:param name="pageTitle" value="Edit Channel" />
...
<div id="primaryContent">
	...
	<div class="breadcrumb">
		<a href="/channel/">Channels</a> &gt; Edit Channel
	</div>
		...
		<form action="/channel/" method="post">
			<table>
				<c:choose>
					...
					<c:otherwise>
						<tr><td class="fieldLabel"><label for="name">Name</label></td><td><input name="name" id="name" size="20" value="${fn:escapeXml(channel.name)}" /></td></tr>
					</c:otherwise>
				</c:choose>

				<tr>
					<c:set var="description" value="${filter.description}" />
				...
				<table>
					...
					<tr>
						...
						<td class="verticalButtons">
							<input type="button" name="filters_rightbtn" class="arrowButton" id="rightButton"/>
							<br />	
						...
						<td class="verticalButtons">
							<input type="button" name="filters_upbtn"  class="arrowButton" id="upButton" />
							<br />	
					...
					<tr>
						<td>
							<select name="outEndpoints_dual" id="outEndpoints_dual" multiple="multiple" size="4" style="width: 200px">
								<c:forEach var="endpoint" items="${endpoints}">
									<c:set var="checkOutboundEndpoint" value="false" />
								
									<c:forEach var="currentOutboundEndpoint" items="${currentOutboundEndpoints}">
										<c:if test="${endpoint.id == currentOutboundEndpoint.id}">
											<c:set var="checkOutboundEndpoint" value="true" />
										</c:if>
									</c:forEach>
									
									<c:if test="${checkOutboundEndpoint == 'false'}">
										<option value="${endpoint.id}"><c:out value="${endpoint.name}" /></option>
									</c:if>
								</c:forEach>	
							</select>
						...
						<td>
							<select name="outEndpoints" id="outEndpoints" multiple="multiple" size="4" class="dualSelect" style="width: 200px">
								<c:forEach var="currentOutboundEndpoint" items="${currentOutboundEndpoints}">
									<option value="${currentOutboundEndpoint.id}"><c:out value="${currentOutboundEndpoint.name}" /></option>
								</c:forEach>									    					
							</select>			    					
						</td>
						<td class="verticalButtons">
