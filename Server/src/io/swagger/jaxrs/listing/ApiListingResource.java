...
public class ApiListingResource {
    ...
    protected synchronized Swagger scan(Application app, ServletConfig sc, Version version) {
        Swagger swagger = null;

        /*
         * Get the scanner from our custom factory, which supports multiple versions
         */
        Scanner scanner = ScannerFactory.getScanner(version);
        LOGGER.debug("using scanner " + scanner);

        if (scanner != null) {
            SwaggerSerializers.setPrettyPrint(scanner.getPrettyPrint());
            swagger = (Swagger) context.getAttribute("swagger" + version);
            ...
            if (classes != null) {
                ...
                context.setAttribute("swagger" + version, swagger);
            }
        }
        initialized = true;
        return swagger;
    ...
    private Version getVersion(UriInfo uriInfo) {
        String[] paths = StringUtils.split(uriInfo.getBaseUri().getPath(), '/');
        if (paths.length > 0) {
            return Version.fromString(paths[paths.length - 1]);
        }
        return null;
    }

    private Swagger process(Application app, ServletConfig sc, HttpHeaders headers, UriInfo uriInfo) {
        Version version = getVersion(uriInfo);
        Swagger swagger = (Swagger) context.getAttribute("swagger" + version);
        if (!initialized) {
